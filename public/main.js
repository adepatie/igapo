...
function animate() {
  ...
  for (let i = 0; i < positions.count; i++) {
    ...
    const wave =
      Math.sin(x * 0.5 + time) * 0.3 + Math.cos(z * 0.3 + time * 0.5) * 0.2;
...
async function loadDynamicTurn() {
  try {
    ...
    if (turnData.animalName && turnData.animalImage) {
      console.log(
        "Setting up educational panel with image:",
        turnData.animalImage
      );
      ...
      document.getElementById("scientific-name").textContent =
        turnData.scientificName || "";
      document.getElementById("educational-fact").textContent =
        turnData.educationalNote || "";
      eduPanel.classList.add("visible");
      console.log("Educational panel should now be visible");
    } else {
      console.log("No animal data, hiding educational panel");
    ...
    turnData.choices.forEach((choice) => {
      ...
      const costText = [];
      if (choice.deltas.morale && choice.deltas.morale < 0)
        costText.push(`Morale ${choice.deltas.morale}`);
      if (choice.deltas.stamina && choice.deltas.stamina < 0)
        costText.push(`Stamina ${choice.deltas.stamina}`);
      if (choice.deltas.supplies && choice.deltas.supplies < 0)
        costText.push(`Supplies ${choice.deltas.supplies}`);
      ...
      button.innerHTML = `
        ...
        ${
          costText.length > 0
            ? `<div style="font-size: 11px; color: #ffaa66; margin-top: 5px;">Cost: ${costText.join(
                " • "
              )}</div>`
            : ""
        }
  ...
  try {
    ...
    if (rewardMessage) {
      ...
      rewardDiv.style.cssText =
        "font-size: 14px; color: #ffdd88; background: rgba(100, 80, 0, 0.4); padding: 12px; border-radius: 6px; border-left: 3px solid #ffaa00; margin-bottom: 10px; text-align: center; font-weight: bold;";
...
function updateUI() {
  const location =
    gameState.route[gameState.progress] ||
    gameState.route[gameState.route.length - 1];
...
function showGameOver(message) {
  ...
  actionsEl.innerHTML = `
    <div id="game-over" style="text-align: center; padding: 20px;">
      <h2 style="color: ${
        gameState.status === "complete" ? "#ffd700" : "#ff8888"
      }; margin-bottom: 15px; font-size: 28px;">${message}</h2>
      ...
      <div style="margin: 20px 0; padding: 15px; background: rgba(0,0,0,0.3); border-radius: 8px;">
        <div style="color: #88ddff; font-size: 14px;">
          <div>📍 Locations Reached: ${gameState.progress + 1}/${
    gameState.route.length
  }</div>
